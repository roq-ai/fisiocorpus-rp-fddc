...
import { deleteInterventionById } from 'apiSdk/interventions';
...
function PatientViewPage() {
  ...
  const interventionHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Patient Physiotherapist Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_patient_physiotherapist_idTouser?.id}`}>
                      {data?.user_patient_physiotherapist_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Patient Pilates Instructor Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_patient_pilates_instructor_idTouser?.id}`}>
                      {data?.user_patient_pilates_instructor_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('intervention', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/interventions/create?patient_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.intervention?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/interventions/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('intervention', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/interventions/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('intervention', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    interventionHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
